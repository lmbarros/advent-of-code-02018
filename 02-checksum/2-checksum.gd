...
# Alright, brute force will do...


var theDifferentIndex = -1

func checkDifferByOne(code1, code2):
    var diffs = 0
    var n = len(code1)
    for i in range(n):
        if code1[i] != code2[i]:
...
func _init():
    ...
    for i in range(n):
         for j in range(i, n):
            if checkDifferByOne(input[i], input[j]):
                print("There it is: " + input[i].substr(0, theDifferentIndex) + input[i].substr(theDifferentIndex + 1, n))
                return
